...
import { CheckCircle2, X, Loader2, AlertCircle, Video, ChevronDown, ChevronUp, Ban } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

export default function UploadQueueIndicator() {
  const { uploads, dismissUpload, cancelUpload } = useUploadQueue();
  const [collapsed, setCollapsed] = useState(false);

  if (uploads.length === 0) return null;

  const activeCount = uploads.filter((u) => u.status === 'uploading' || u.status === 'cancelling').length;
      ...
      <div className="bg-white border rounded-lg shadow-xl overflow-hidden">
        ...
        {!collapsed && (
          <div className="max-h-80 overflow-y-auto">
            {uploads.map((u) => {
              const isInFlight = u.status === 'uploading' || u.status === 'cancelling';
              return (
                <div
                  key={u.id}
                  className={cn(
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 truncate" title={u.productName}>
                      ...
                      {u.status === 'cancelling' && ' · Cancelling...'}
                  ...
                  {u.status === 'uploading' && (
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-7 px-2 text-xs flex-shrink-0 text-red-600 hover:text-red-700 hover:bg-red-50"
