-- Merchant System Implementation - FINAL VERSION
...
-- Update customer_type check constraint (already exists but ensure merchant is included)
ALTER TABLE public.customer_profiles
...
-- ============================================
-- 4. BULK PRICING TIERS (for component_library)
-- ============================================
...
CREATE TABLE IF NOT EXISTS public.bulk_pricing_tiers (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE(component_id, min_quantity)
);
...
CREATE TABLE IF NOT EXISTS public.merchant_favorites (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE(customer_id, component_id)
);
...
CREATE INDEX IF NOT EXISTS idx_bulk_pricing_component ON public.bulk_pricing_tiers(component_id) WHERE is_active = true;
...
-- Admin check: Check if current auth user has matching customer_profile that is linked to admin
...
CREATE POLICY "admin_manage_merchant_codes" ON public.merchant_codes
  FOR ALL USING (
    EXISTS (
      ...
      AND cp.username IN (SELECT username FROM public.admin_profiles WHERE is_active = true)
    )
  );
...
CREATE POLICY "customers_manage_own_registration" ON public.merchant_registrations
  FOR ALL USING (
    EXISTS (
      ...
      WHERE id = merchant_registrations.customer_id AND user_id = auth.uid()
    )
  );
...
CREATE POLICY "customers_view_own_wallet" ON public.merchant_wallets
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.customer_profiles
      WHERE id = merchant_wallets.customer_id AND user_id = auth.uid()
    )
  );
...
CREATE POLICY "customers_view_own_transactions" ON public.wallet_transactions
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.customer_profiles
      WHERE id = wallet_transactions.customer_id AND user_id = auth.uid()
    )
  );
...
CREATE POLICY "customers_view_own_promotion_usage" ON public.promotion_usage
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.customer_profiles
      WHERE id = promotion_usage.customer_id AND user_id = auth.uid()
    )
  );
...
CREATE POLICY "customers_manage_own_favorites" ON public.merchant_favorites
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.customer_profiles
      WHERE id = merchant_favorites.customer_id AND user_id = auth.uid()
    )
  );
...
CREATE POLICY "customers_view_own_purchase_history" ON public.merchant_purchase_history
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.customer_profiles
      WHERE id = merchant_purchase_history.customer_id AND user_id = auth.uid()
    )
  );
...
-- Get component price function (for component_library)
CREATE OR REPLACE FUNCTION public.get_component_price(
  p_component_id UUID,
  p_customer_id UUID,
  p_quantity INTEGER
)
...
BEGIN
  ...
  IF v_customer_type = 'merchant' OR v_pricing_type = 'wholesale' THEN
    SELECT merchant_price INTO v_base_price FROM public.component_library WHERE id = p_component_id;
  ELSE
    SELECT normal_price INTO v_base_price FROM public.component_library WHERE id = p_component_id;
  END IF;
...
COMMENT ON TABLE public.bulk_pricing_tiers IS 'Quantity-based pricing tiers for bulk purchases on components';
...
COMMENT ON TABLE public.merchant_favorites IS 'Merchant favorite components for quick reorder';
COMMENT ON TABLE public.merchant_purchase_history IS 'Aggregated purchase history per component';

-- Success message
DO $$
BEGIN
  RAISE NOTICE '✅ Merchant system installed successfully!';
  RAISE NOTICE '📋 Tables created: merchant_codes, merchant_registrations, merchant_wallets, wallet_transactions';
  RAISE NOTICE '📦 Component pricing: bulk_pricing_tiers, merchant_favorites, merchant_purchase_history';
  RAISE NOTICE '🎁 Promotions: merchant_promotions, promotion_usage';
  RAISE NOTICE '🔐 4 demo codes: MERCHANT2024, DEALER100, WORKSHOP50, VIP001';
  RAISE NOTICE '🎉 2 promotions: NEWMERCHANT50, BULK20';
  RAISE NOTICE '';
  RAISE NOTICE '🚀 Next: Test at /merchant-register with code MERCHANT2024';
END $$;
