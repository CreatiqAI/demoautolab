-- =====================================================
-- CATALOG SMART SEARCH — match component SKUs
-- Extends catalog_search_products so a query can match the
-- component_sku of any component inside a product (not just the
-- component name). Mirrors the chatbot search, which already
-- matches component_library.component_sku. Without this, searching
-- a part's SKU returned no results.
-- =====================================================

-- Trigram index backing the component_sku ILIKE / similarity matching.
CREATE INDEX IF NOT EXISTS idx_component_library_sku_trgm
  ON component_library USING gin (lower(coalesce(component_sku, '')) gin_trgm_ops);
  ...
  WITH base AS (
    SELECT
      ...
      CASE WHEN v_query = '' THEN 0::REAL ELSE (
          ...
          -- Component name OR component SKU (whole query)
          + CASE WHEN EXISTS (
          ...
          -- Component name OR component SKU (per token)
          + COALESCE((
