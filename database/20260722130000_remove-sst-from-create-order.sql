-- Remove SST (6%): create_order_with_items no longer computes or adds tax.
-- Reproduced from the FOC-guard definition (20260623130000); only the tax line
-- changed (v_seller_tax := 0) and the total no longer includes tax. orders.tax
-- stores 0 for new orders; historical orders keep their stored tax. Applied 2026-07-22.
...
BEGIN
  ...
  LOOP
    ...
    -- SST removed: no tax on orders.
    v_seller_tax := 0;

    v_seller_total := v_seller_subtotal - v_seller_voucher + v_seller_shipping_fee;
