-- Complete Inventory Management System Migration
...
INSERT INTO public.inventory (component_sku, component_name, category_id, supplier_id, current_stock, min_stock_level, max_stock_level, reorder_point, unit_cost, selling_price, location) VALUES
  ('BM-026N', 'BMW X3 9 Inch Casing',
   (SELECT id FROM categories WHERE name = '9/10 Inch Casing Android' LIMIT 1),
  ...
  ('NS-PLUS-01', 'NinjaShades Plus Magnetic Sunshade',
   (SELECT id FROM categories WHERE name = 'Car Magnetic Sunshades (NinjaShades Plus)' LIMIT 1),
  ...
  ('HD-AUDIO-12', '12.5 Inch Android Head Unit',
   (SELECT id FROM categories WHERE name = '12.5 Inch Casing Android' LIMIT 1),
  ...
  ('LED-AMB-001', 'Ambient LED Light Strip',
   (SELECT id FROM categories WHERE name = 'Interior Lighting' LIMIT 1),
...
-- Trigger initial stock level check for existing inventory
DO $$
DECLARE
  inv_record RECORD;
BEGIN
  FOR inv_record IN SELECT * FROM inventory LOOP
    PERFORM check_stock_levels() WHERE NEW = inv_record;
  END LOOP;
END $$;