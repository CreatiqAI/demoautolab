-- Complete Inventory Management System Migration - FIXED VERSION
...
CREATE POLICY IF NOT EXISTS "Enable all for authenticated users" ON suppliers FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Enable all for authenticated users" ON inventory FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Enable all for authenticated users" ON stock_movements FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Enable all for authenticated users" ON restock_orders FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Enable all for authenticated users" ON restock_order_items FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Enable all for authenticated users" ON stock_alerts FOR ALL USING (auth.role() = 'authenticated');
