...
serve(async (req) => {
  ...
  try {
    ...
    if (!contentType.startsWith('image/')) {
      return new Response(JSON.stringify({ error: `not an image: content-type was "${contentType}". Make sure the Drive folder is shared "Anyone with the link".` }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    const buf = new Uint8Array(await resp.arrayBuffer());
    if (buf.byteLength > MAX_BYTES) {
      return new Response(JSON.stringify({ error: `image too large (>${MAX_BYTES / 1024 / 1024}MB)` }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
